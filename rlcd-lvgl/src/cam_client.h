...
/* 9-4：M1 键盘吉他控制（GTR:STRUM / GTR:CHORD / GTR:GROUP，USB 与 WiFi 双通道）。
 * take_gtr_cmd：loop() 轮询取走，返回 0=拨弦 1=下一个和弦 2=切练习组，无命令 -1。
 * 执行约定：main loop 在 Lvgl_lock 内先保证位于吉他页（不在则 ui_goto_page(1)
 * 自动跳页）再调 ui_ambient_xxx；成功后调 send_gtr_ack 回 ACK:GTR:xxx。
 * 与 take_page_cmd 相同语义：取走即清空，同一命令只执行一次。 */
