...
# ============================================================
#  股票指数行情模块（腾讯单源，免费、无需 key）
#  - 上证 sh000001 / 沪深300 sh000300 / 创业板 sz399006 → 腾讯行情
#  - AI 行：sz159819 人工智能ETF（8-31 起）。★ 曾用东方财富 930713
#    CS人工智能指数（点位 5955 量级），而 ESP32 固件 8-17 起已改用
#    159819 ETF（价格 1.7xx 量级），固件会主动跳过 code=930713 的行
#    → 后端一旦走回退路径，AI 行永远拿不到数据，屏幕上一直显示 "--"。
#    后端与固件统一为 sz159819 后，回退路径同样能出 AI 行。
...
STOCK_CODES = [
    ...
    ("sh000300", "300"),
    ("sz399006", "创板"),
    ("sz159819", "AI"),       # 人工智能ETF（与 ESP32 固件 TX_IDX 保持一致）
]
...
def _fetch_stock_quotes() -> list:
    """抓取腾讯 4 个标的（3 指数 + 人工智能ETF）合并为 [{name,code,value,pct,up}]。
    ...
    字段（~ 分隔，索引）：
      ...
      [31] 涨跌额, [32] 涨跌幅(%)
    指数与 ETF 的字段布局一致，同一套解析即可覆盖。
    """
    out = []

    # ---- 腾讯：3 个常见指数 + 人工智能ETF（sz159819） ----
    codes = ",".join(c for c, _ in STOCK_CODES)
    url = STOCK_API_URL.format(codes=codes)
    # 8-31：腾讯 HTTPS 在办公网偶发 SSL UNEXPECTED_EOF（TLS 中途断开）。
    # 单次失败会让 fetch_stocks 回吐磁盘陈旧缓存 → 设备端拿到过期行情
    # （历史上还吐过 code=930713 的旧 AI 行，被固件跳过 → 屏幕 "--"）。
    # 重试 3 次、短退避，把偶发抖动挡在回退之前。
    raw = None
    last_err = "unknown"
    ...
    if raw is None:
        raise RuntimeError(f"腾讯行情三次均失败: {last_err}")
...
def create_app(target_name: str = "你的姓名", city: str = "北京"):
    ...
    def get_stocks():
        """
        四行行情：上证/沪深300/创业板指/AI(159819 人工智能ETF)（腾讯行情 API，免费）。
